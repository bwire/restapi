...
handlers.index = function (data, callback) {
  ...
  if (data.method === 'get') {
    ...
    let templateData = {
      ...
      'head.description': 'We offer free, simple uptime monitoring for HTTP/HTTPS sites of all kinds. When your site goes down we\'ll send you a text tolet you know',
      'body.class': 'index'
    }

    // Read in a template as a string
    _helpers.getTemplate('index', templateData, (error, htmlString) => {
      if (!error && htmlString) {
        // Add the universal header and footer
        _helpers.addUniversalTemplates(htmlString, templateData, (error, str) => {
          if (!error && str) {
...
handlers.accountCreate = function (data, callback) {
  ...
  if (data.method === 'get') {
    ...
    // Read in a template as a string
    _helpers.getTemplate('accountCreate', templateData, (error, htmlString) => {
      if (!error && htmlString) {
        // Add the universal header and footer
        _helpers.addUniversalTemplates(htmlString, templateData, (error, str) => {
          if (!error && str) {
...
handlers.public = (data, callback) => {
  if (data.method === 'get') {
    // get the filename being requeted
          ...
          if (!error && data) {
            _data.delete('users', phone, (error) => {
              if (!error) {
                ...
                const checksToDelete = data.checks.length
                if (checksToDelete) {
    ...
    _data.read('users', phone, (error, userData) => {
      if (!error && userData) {
        ...
        if (hashedPassword === userData.password) {
          ...
          callback(400, {'Error': 'The password provided did not matches the specified user\'s strored'})
        };
      } else {
